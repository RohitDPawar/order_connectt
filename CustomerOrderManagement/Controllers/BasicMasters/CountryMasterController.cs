...
namespace CustomerOrderManagement.Controllers.BasicMasters
{
  ...
  {
    private readonly CountryMasterBAL countryBAL;
    // Constructor injection for LoginBAL
    public CountryMasterController(CountryMasterBAL CountryMasterBal)
    {
      countryBAL = CountryMasterBal;
    }

    //THIS FUNCTION USED FOR LOAD INDEX OF COUNTRY MASTER
    public IActionResult Index()
    {
      //THIS US USEFD FOR GET ALL ADDED COUNTRY
      return View();
    }

    public IActionResult GetCountryData([FromBody] Dictionary<string, object> data)
    {
      try
      {
        var items = countryBAL.GetAllCountry(data);

        if (items.Tables.Count >= 2)
        {
          var countryData = items.Tables[0];
    ...
    // THIS FUNCTION USED FOR SAVE COUNTRY DETAILS
    public IActionResult SaveAddCountry()
    {

      var Form = HttpContext.Request.Form;
      string CountryData = countryBAL.CheckDuplicateRecord("country_name", Form["Name"].ToString());
      if (CountryData == "")
      {
        countryBAL.SaveAddCountry(Form);
        TempData["Message"] = "Country Added Successfully !!!!!";
      }
      else
      {
        TempData["Message"] = "Country Already Exit !!!!!";
      }
      return RedirectToAction("Index", "CountryMaster");
    }

    //THIS IS USED FOR GET COUNTRY FOR EDIT
    public IActionResult EditCountry(int ID, string Flag)
    {
      var jsondata = "";
      Dictionary<string, object> Param = new Dictionary<string, object>();
      try
      {
        Param.Add("CountryData", countryBAL.GetEditCountryData(ID).Tables[0]);
        Param.Add("Flag", Flag);
      }
      catch (Exception ex)
      {
    ...
    public ActionResult SaveEditCountry()
    {
      var Form = HttpContext.Request.Form;

      countryBAL.SaveEditCountry(Form);
    ...
    {
      string result = countryBAL.CheckDuplicateRecord(ColName, value);
      return Json(result);
    }
    ...
    public IActionResult GetHistoryData(int id)
    {
      ...
      Param.Add("HistoryData", countryBAL.GetHistoryData(id).Tables[0]);
      jsondata = JsonConvert.SerializeObject(Param);
      return Json(jsondata);
    }

    public IActionResult ExportdataExcelCountry(string selectedRowIds, string searchValue, string tablename, string coloumname)
    {
      try
      {
        ...
        var ds = countryBAL.GetFilterCountryData(selectedRowIds, searchValue);
        ...
        using (var memoryStream = new MemoryStream())
        {
          ...
          using (var package = new ExcelPackage(memoryStream))
          {
            ...
            var worksheet = package.Workbook.Worksheets.Add("CountryData");
          ...
          // Set the filename
          var fileName = "CountryData_" + DateTime.Now.ToString("yyyyMMddHHmmss") + ".xlsx";
