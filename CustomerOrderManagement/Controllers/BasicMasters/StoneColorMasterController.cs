...
{
  public class StoneColorMasterController : Controller
  {
    private readonly StoneColorMasterBAL StoneBal;
    private readonly GlobalSessionBAL GlobalBal;
    // Constructor injection for LoginBAL
    public StoneColorMasterController(StoneColorMasterBAL StoneMasterBal, GlobalSessionBAL globalBal)
    {
      StoneBal = StoneMasterBal;
      GlobalBal = globalBal;
    }
    public IActionResult Index()
    {
      ViewBag.GetAllTenants = StoneBal.GetTenantDetails().Tables[0].AsEnumerable();
      return View();
    }

    public IActionResult GetStoneColorData([FromBody] Dictionary<string, object> data)
    {
      try
      {
        var items = StoneBal.GetAllStoneColor(data);

        if (items.Tables.Count >= 2)
        {
          var stoneColorData = items.Tables[0];

          var paginationInfo = items.Tables[1];

          var param = new Dictionary<string, object>
            {
                { "StoneColorData", stoneColorData },
    ...
    public IActionResult SaveAddStoneColor()
    {
      var Form = HttpContext.Request.Form;
      StoneBal.SaveAddStoneColor(Form);
      TempData["Message"] = "Stone Color Added Successfully !!!!!";
      return RedirectToAction("Index", "StoneColorMaster");
    }
    // THIS FUNCTION IS USED TO EDIT THE CITY MASTER
    public IActionResult EditStoneColor(int ID, string Flag)
    {
      var jsondata = "";
      Dictionary<string, object> Param = new Dictionary<string, object>();
      try
      {
        Param.Add("StoneColorData", StoneBal.GetEditStoneColorData(ID).Tables[0]);
        Param.Add("Flag", Flag);
      }
      catch (Exception ex)
      {
    ...
    public IActionResult GetHistoryData(int id)
    {
      ...
      Param.Add("HistoryData", StoneBal.GetHistoryData(id).Tables[0]);
      jsondata = JsonConvert.SerializeObject(Param);
      return Json(jsondata);
    }

    //THIS FUNCTION USED FOR UPDATE CITY DETAILS
    public IActionResult SaveEditStoneColor()
    {
      var Form = HttpContext.Request.Form;
      StoneBal.SaveEditStoneColor(Form);
      TempData["Message"] = "Stone Color Updated Successfully !!!!!";
      return RedirectToAction("Index", "StoneColorMaster");
    }
    ...
    {
      string result = StoneBal.CheckDuplicateRecord(ColName, value);
      return Json(result);
    }

    public IActionResult ExportdataExcelStoneColor(string selectedRowIds, string searchValue, string tablename, string coloumname)
    {
      try
      {
        ...
        var ds = StoneBal.GetFilterStoneColorData(selectedRowIds, searchValue);
        ...
        using (var memoryStream = new MemoryStream())
        {
          ...
          using (var package = new ExcelPackage(memoryStream))
          {
            ...
            var worksheet = package.Workbook.Worksheets.Add("StoneColorData");
          ...
          // Set the filename
          var fileName = "StoneColorData_" + DateTime.Now.ToString("yyyyMMddHHmmss") + ".xlsx";
    ...
    [HttpPost]
    public async Task<IActionResult> UploadStoneColor(IFormFile file)
    {
      try
      {
        ...
        using (var package = new ExcelPackage(file.OpenReadStream()))
        {
          ...
          var requiredHeaders = new[] { "color_name","remark", "is_active" };
          ...
          for (int row = 2; row <= worksheet.Dimension.End.Row; row++)
          {
            ...
            StoneBal.SaveImportDataStoneColor(dataRow["color_name"].ToString(), dataRow["remark"].ToString(), dataRow["is_active"].ToString(), uniqueId);
          }

          DataSet result = GlobalBal.GetImportErrorData(uniqueId, GlobalBal.GetClientIpAddress(), "sp_ValidateAndSaveStoneColorData");
          DataTable errorDataTable = result.Tables[0];

          if (errorDataTable != null && errorDataTable.Rows.Count > 0)
          {
            var errorWorksheet = outputPackage.Workbook.Worksheets.Add("Stone Color Import Errors");
            ...
            byte[] fileBytes = memoryStream.ToArray();
            string fileName = "StoneColorInvalidData_" + DateTime.Now.ToString("dd-MM-yy_HHmmss") + ".xlsx";
    ...
    public IActionResult DownloadSampleSheet()
    {
      ...
      var fileUrl = "/FileStorage/Sample_Sheet_Order/stone_color_master_format.xlsx";
