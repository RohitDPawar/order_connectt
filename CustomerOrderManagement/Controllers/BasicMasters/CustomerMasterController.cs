...
namespace CustomerOrderManagement.Controllers.BasicMasters
{
  ...
  {
    private readonly CustomerMasterBAL CustomerBAL;
    private readonly TenantMasterBAL TenantBAL;
    private readonly GlobalSessionBAL GlobalBal;
    // Constructor injection for LoginBAL
    public CustomerMasterController(CustomerMasterBAL CustomerBaL, TenantMasterBAL tenantBAL, GlobalSessionBAL globalBal)
    {
    ...
    {
      ViewBag.CountryID = CustomerBAL.GetCountryID().Tables[0].AsEnumerable();
      ViewBag.StateID = CustomerBAL.GetStateID().Tables[0].AsEnumerable();
      ViewBag.CityID = CustomerBAL.GetCityID().Tables[0].AsEnumerable();
      ViewBag.TenantID = CustomerBAL.GettenantID().Tables[0].AsEnumerable();
      return View();
    }
    ...
    {
      var allGroups = CustomerBAL.GetFilteredCountry(SearchItem).Tables[0].AsEnumerable();
    ...
    public IActionResult GetCustomerData([FromBody] Dictionary<string, object> data)
    {
      try
      {
        var items = CustomerBAL.GetCustomerData(data);

        if (items.Tables.Count >= 2)
        {
          var customerData = items.Tables[0];
    ...
    public IActionResult SaveAddCustomer()
    {
      var Form = HttpContext.Request.Form;
      CustomerBAL.SaveAddCustomer(Form);
    ...
    public IActionResult EditCustomer(int ID, string Flag)
    {
      var jsondata = "";
      Dictionary<string, object> Param = new Dictionary<string, object>();

      Param.Add("CustomerData", CustomerBAL.GetEditCustomerData(ID).Tables[0]);
      Param.Add("Flag", Flag);
      jsondata = JsonConvert.SerializeObject(Param);
      return Json(jsondata);
    }
    //THIS FUNCTION USED FOR UPDATE Vendor DETAILS
    public IActionResult SaveEditCustomer()
    {
      var Form = HttpContext.Request.Form;
      CustomerBAL.SaveEditCustomer(Form);
    ...
    {
      string result = CustomerBAL.CheckDuplicateRecord(ColName, Username);
      return Json(result);
    }
    ...
    public IActionResult GetHistoryData(int id)
    {
      var jsondata = "";
      Dictionary<string, object> Param = new Dictionary<string, object>();
      Param.Add("HistoryData", CustomerBAL.GetHistoryData(id).Tables[0]);
      jsondata = JsonConvert.SerializeObject(Param);
      return Json(jsondata);
    }
    ...
    {
      var allGroups = CustomerBAL.GetStateNames(CountryId, SearchItem).Tables[0].AsEnumerable();
    ...
    {
      var allGroups = CustomerBAL.GetCityNames(StateId, SearchItem).Tables[0].AsEnumerable();
    ...
    public IActionResult ExportdataExcelCustomers(string selectedRowIds, string searchValue, string tablename, string coloumname)
    {
      try
      {
        ...
        var ds = CustomerBAL.GetFilterCustomersData(selectedRowIds, searchValue);
        ...
        using (var memoryStream = new MemoryStream())
        {
          ...
          using (var package = new ExcelPackage(memoryStream))
          {
            ...
            var worksheet = package.Workbook.Worksheets.Add("CustomersData");
          ...
          // Set the filename
          var fileName = "CustomersData_" + DateTime.Now.ToString("yyyyMMddHHmmss") + ".xlsx";
    ...
    [HttpPost]
    public async Task<IActionResult> UploadCustomerMaster(IFormFile file)
    {
      try
      {
        ...
        using (var package = new ExcelPackage(file.OpenReadStream()))
        {
          ...
          var requiredHeaders = new[] { "customer_name", "mobile_number", "email_id", "address", "area", "pincode", "country", "state", "city", "remark", "is_active" };
          ...
          for (int row = 2; row <= worksheet.Dimension.End.Row; row++)
          {
            ...
            para.Add("customer_name", dataRow["customer_name"].ToString());
            para.Add("mobile_no", dataRow["mobile_number"].ToString());
            para.Add("email_id", dataRow["email_id"].ToString());
            ...
            para.Add("country_name", dataRow["country"].ToString());
            para.Add("state_name", dataRow["state"].ToString());
            para.Add("city_name", dataRow["city"].ToString());
            para.Add("remark", dataRow["remark"].ToString());
            para.Add("is_active", dataRow["is_active"].ToString());
            para.Add("unique_id", uniqueId);

            CustomerBAL.SaveAddCustomerImportTemp(para);
          }

          DataSet result = GlobalBal.GetImportErrorData(uniqueId, GlobalBal.GetClientIpAddress(), "sp_ValidateAndSaveCustomerData");
          DataTable errorDataTable = result.Tables[0];

          if (errorDataTable != null && errorDataTable.Rows.Count > 0)
          {
            var errorWorksheet = outputPackage.Workbook.Worksheets.Add("Customer Import Errors");
            ...
            byte[] fileBytes = memoryStream.ToArray();
            string fileName = "CustomerInvalidData_" + DateTime.Now.ToString("dd-MM-yy_HHmmss") + ".xlsx";
    ...
    public IActionResult DownloadSampleSheet()
    {
      ...
      var fileUrl = "/FileStorage/Sample_Sheet_Order/Customer_data_import.xlsx";
